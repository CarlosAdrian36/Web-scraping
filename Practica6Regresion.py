...
# ||||||||||||||||||| Precio de venta y area de sotano|||||||||||||||||
var = 'TotalBsmtSF'
data = pd.concat([df['SalePrice'], df[var]], axis=1)
data.plot.scatter(x=var, y='SalePrice', ylim=(0, 800000))
# plt.show()

# |||||||||||||||||||||Precio venta y calidad de la construccion (Categorica)||||||||||||||||||||||
var = 'OverallQual'
data = pd.concat([df['SalePrice'], df[var]], axis=1)
data.head()
